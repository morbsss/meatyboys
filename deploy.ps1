...
SSH-Run "mkdir -p ${VM_APP_DIR}/templates ${VM_APP_DIR}/public ${VM_APP_DIR}/scripts ${VM_APP_DIR}/data ${VM_APP_DIR}/playerfiles ${VM_APP_DIR}/nginx"
...
# Static data files, deployed with the app - not generated by scripts
scp -P $VM_SSH_PORT data\currentseason.json "${VM}:${VM_APP_DIR}/data/"
scp -P $VM_SSH_PORT data\leaguetable.json   "${VM}:${VM_APP_DIR}/data/"
scp -P $VM_SSH_PORT data\fixtures.json      "${VM}:${VM_APP_DIR}/data/"
...
# Strip any CRLF line endings from deploy.sh on the VM (bash chokes on \r).
# Idempotent: no-op if already LF.
